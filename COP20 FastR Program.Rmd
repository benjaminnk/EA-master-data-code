...
```
##Load required programs and set your computer's Working  Director to where the files are located on your computer. In this case, I set my working directory to my SIEI COP20 folder.
```{r,results="Hide"}
library("tidyverse")
library(fastR)
library(tameDP)
library(purrr)
setwd("~/Credence/on boarding/SIEI/COP20")
```
...
```{r,results="Hide"}
write_csv(df, "Budget.ER.MER.4.30.2020.csv")
